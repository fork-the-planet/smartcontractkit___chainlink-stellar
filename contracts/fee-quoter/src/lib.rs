...
impl FeeQuoterContract {
    ...
    // ========================================
    // Authorized Caller Management (delegated to common-authorization)
    // ========================================

    /// Add an authorized caller (owner only).
    /// Note: The common-authorization library's `add_callers` internally
    /// calls `Ownable::require_owner`, so we do not check ownership here
    /// to avoid a duplicate auth check.
    ...
    /// Note: The common-authorization library's `remove_callers` internally
    /// calls `Ownable::require_owner`, so we do not check ownership here
    /// to avoid a duplicate auth check.
    ...
    pub fn get_authorized_callers(env: Env) -> Result<Vec<Address>, FeeQuoterError> {
        Self::require_initialized(&env)?;
        Ok(AuthorizedCallers::get_callers(&env))
    }

    // ========================================
    // Owner Management (delegated to common-authorization)
    // ========================================
    ...
    pub fn accept_ownership(env: Env) -> Result<(), FeeQuoterError> {
        Self::require_initialized(&env)?;
    ...
    pub fn get_pending_owner(env: Env) -> Result<Option<Address>, FeeQuoterError> {
        Self::require_initialized(&env)?;
        Ok(Ownable::get_pending_owner(&env))
    }

    /// Get the current owner.
    pub fn owner(env: Env) -> Result<Address, FeeQuoterError> {
        Self::require_initialized(&env)?;
        Ownable::get_owner(&env).ok_or(FeeQuoterError::NotInitialized)
    }
