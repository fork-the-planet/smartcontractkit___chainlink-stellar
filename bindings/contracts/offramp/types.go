...
// ToScVal converts SourceChainConfig to an xdr.ScVal for contract calls.
func (s SourceChainConfig) ToScVal() (xdr.ScVal, error) {
	return scval.BuildStructScVal(map[string]xdr.ScVal{
		...
		"on_ramps":           scval.BytesSliceToScVal(s.OnRamps),
...
func SourceChainConfigFromScVal(val xdr.ScVal) (*SourceChainConfig, error) {
	...
	for _, entry := range *scMap {
		...
		case "on_ramps":
			vec, ok := entry.Val.GetVec()
			if !ok || vec == nil {
				return nil, fmt.Errorf("on_ramps is not a vec")
			}
			result.OnRamps = make([][]byte, len(*vec))
			for i, item := range *vec {
				v, ok := item.GetBytes()
				if !ok {
					return nil, fmt.Errorf("vec item is not bytes")
				}
				result.OnRamps[i] = []byte(v)
			}
		case "router":
			v, err := scval.AddressFromScVal(entry.Val)
			if err != nil {
...
// ToScVal converts SourceChainConfigArgs to an xdr.ScVal for contract calls.
func (s SourceChainConfigArgs) ToScVal() (xdr.ScVal, error) {
	return scval.BuildStructScVal(map[string]xdr.ScVal{
		"default_ccvs":          scval.AddressSliceToScVal(s.DefaultCcvs),
		"is_enabled":            scval.BoolToScVal(s.IsEnabled),
		"lane_mandated_ccvs":    scval.AddressSliceToScVal(s.LaneMandatedCcvs),
...
const (
	...
	MessageExecutionStateSuccess    MessageExecutionState = 2
	MessageExecutionStateFailure    MessageExecutionState = 3
)

// ToScVal converts MessageExecutionState to an xdr.ScVal.
func (e MessageExecutionState) ToScVal() (xdr.ScVal, error) {
	return scval.Uint32ToScVal(uint32(e)), nil
}
...
func MessageExecutionStateFromScVal(val xdr.ScVal) (MessageExecutionState, error) {
	v, ok := val.GetU32()
	if !ok {
		return 0, fmt.Errorf("expected u32 for MessageExecutionState enum")
	}
	return MessageExecutionState(v), nil
}
