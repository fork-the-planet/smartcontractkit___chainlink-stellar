...
// FeeResponse represents the FeeResponse struct from the contract.
...
// ToScVal converts FeeResponse to an xdr.ScVal for contract calls.
func (s FeeResponse) ToScVal() (xdr.ScVal, error) {
	return scval.BuildStructScVal(map[string]xdr.ScVal{
		...
		"fee":                 scval.Uint32ToScVal(s.Fee),
	})
}

// FeeResponseFromScVal parses an xdr.ScVal into FeeResponse.
func FeeResponseFromScVal(val xdr.ScVal) (*FeeResponse, error) {
	scMap, ok := val.GetMap()
	if !ok || scMap == nil {
	...
	for _, entry := range *scMap {
		...
		case "fee":
			v, ok := entry.Val.GetU32()
			if !ok {
				return nil, fmt.Errorf("fee is not u32")
			}
			result.Fee = uint32(v)
		}
	}
