...
`deployment/ccip/stellardeploy` runs Soroban operations on a **caller-supplied** CLDF `Bundle` (same as EVM/Solana: the parent changeset bundle from `ExecuteSequence`, or `operations.NewBundle` in standalone tests). [`DeployStellarCCIPContracts`](../ccip/stellardeploy/deploy.go) and [`NewStellarCCIPDeploySession`](../ccip/stellardeploy/session.go) require that bundle explicitly; they do not create a parallel in-memory bundle. Shared datastore helpers and contract-type constants live in [`deployment/ccip`](../ccip/). The [`deployment/ccip/devenv`](../ccip/devenv/) package keeps backward-compatible aliases for older import paths. **FeeQuoter `update_prices`** still uses the generated client directly because `PriceUpdates` embeds `scval.U128` types that are not CLDF JSON-serialization friendly.
